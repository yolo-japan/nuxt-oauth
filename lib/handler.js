...
Handler.prototype.createSession = function createSession () {
  ...
  const session = sessions({
    ...
    activeDuration: this.opts.activeDuration * 1000,
...
Handler.prototype.updateToken = async function updateToken () {
  ...
  try {
    ...
    let newExpiresIn = new Date(token.expires).getTime() - new Date().getTime()
    if (Number.isInteger(this.opts.tokenExpirationMargin)) {
      newExpiresIn -= this.opts.tokenExpirationMargin * 1000
    }
    newToken.expiresIn(Math.floor(newExpiresIn / 1000))
